...
def finished():
    while True:
        yield sync(request=BEvent("FINISHED"))


@thread
def control_tank():
    while True:
        if (TARGET.TEMP - 0.1 < TANK.water_temperature < TARGET.TEMP + 0.1) and \
                (TANK.water_level == TARGET.LEVEL):
            yield sync(
                waitFor=All(),
                block=EventSetList([BEvent("HOT"), BEvent("COLD"), BEvent("DRAIN")]),
            )
        elif TANK.water_level > TARGET.LEVEL:
            yield sync(
                waitFor=All(),
                block=EventSetList([BEvent("HOT"), BEvent("COLD"), BEvent("FINISHED")]),
            )
        elif TANK.water_temperature < TARGET.TEMP:
            yield sync(
                waitFor=All(),
                block=EventSetList([BEvent("COLD"), BEvent("DRAIN"), BEvent("FINISHED")]),
            )
        elif TANK.water_temperature >= TARGET.TEMP:
            yield sync(
                waitFor=All(),
                block=EventSetList([BEvent("HOT"), BEvent("DRAIN"), BEvent("FINISHED")]),
            )
...
if __name__ == "__main__":
    def poll_tank_data() -> Dict[str, Dict[str, str | int | bool | Callable]]:
        return {
            "TargetLevel":
                {"value": TARGET.LEVEL, "editable": True, "unit": "l", "callback": set_level},
            "TargetTemperature":
                {"value": TARGET.TEMP, "editable": True, "unit": "°C", "callback": set_temp},
            "Level":
                {"value": round(TANK.water_level, ndigits=3), "editable": False, "unit": "l"},
            "Temperature":
                {"value": round(TANK.water_temperature, ndigits=3), "editable": False, "unit": "°C"},
        }
