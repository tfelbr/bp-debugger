...
def control_drain():
    while True:
        if TANK.water_level > TARGET.LEVEL:
            yield sync(
                block=EventSetList([BEvent("HOT"), BEvent("COLD")]),
                waitFor=BEvent("DRAIN"),
            )
        else:
            yield sync(
                waitFor=All(),
...
def control_temp():
    while True:
        if (TARGET.TEMP - 0.1 < TANK.water_temperature < TARGET.TEMP + 0.1) and (TARGET.LEVEL == TANK.water_level):
            yield sync(
                waitFor=All(),
                block=All(),
            )
...
if __name__ == "__main__":
    ...
    b_program = BProgram(
        bthreads=[
            ...
            listener.mark_name("ControlDrain", control_drain()),
        ],
